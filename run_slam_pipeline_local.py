...
@click.option('--slam_setting_file', type=str, default=None, help='Path to ORB-SLAM camera setting YAML file')
...
def main(session_dir, calibration_dir, slam_setting_file, camera_intrinsics, aruco_config):
    ...
    for session in session_dir:
        ...
        print("############# 02_create_map ###########")
        script_path = script_dir.joinpath("02_create_map_local.py")
        assert script_path.is_file()
        ...
        print("############# 03_batch_slam ###########")
        script_path = script_dir.joinpath("03_batch_slam_local.py")
        assert script_path.is_file()
        cmd = [
            'python', str(script_path),
